...
def create_ch_creds_secret(yc_sdk, folder_id, ch_user, ch_pass):
    return create_secret(yc_sdk, folder_id, 'ch-creds', {
        'user': ch_user,
        'pass': ch_pass
    })
...
def main():
    ...
    flag_parser.add_argument("--ch-user", type=str, required=True)
    flag_parser.add_argument("--ch-pass", type=str, required=True)
    ...
    ch_creds_secret_id = create_ch_creds_secret(yc_sdk, args.yc_folder_id, args.ch_user, args.ch_pass)
    ch_secret_id = create_ch_secret(yc_sdk, args.yc_folder_id)
    print(f'Telegram secret id: {tg_secret_id}')
    print(f'Clickhouse creds secret id: {ch_creds_secret_id}')
    print(f'Clickhouse cluster secret id: {ch_secret_id}')
